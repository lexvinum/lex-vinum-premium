...
function getLimit(searchParams: URLSearchParams, fallback: number): number {
...
function parseCountries(searchParams: URLSearchParams): string[] {
...
function getRegionFallback(
  ...
  const regionMap: Record<string, { lat: number; lng: number }> = {
    ...
    monteregie: { lat: 45.401, lng: -73.081 },
    estrie: { lat: 45.4042, lng: -71.8929 },
    "cantons de l est": { lat: 45.3, lng: -72.0 },
    "eastern townships": { lat: 45.3, lng: -72.0 },
    lanaudiere: { lat: 46.0, lng: -73.4 },
    laurentides: { lat: 46.15, lng: -74.2 },
    "centre du quebec": { lat: 46.05, lng: -72.0 },
    "ile d orleans": { lat: 46.91, lng: -71.02 },
...
function hasWorldOrigin(
  ...
  return Boolean(normalizeValue(country) || normalizeValue(region));
}
...
function matchesSelectedCountries(
  ...
  return selectedCountries.some(
    (selectedCountry) => normalizeValue(selectedCountry) === normalizedCountry
  );
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const selectedCountries = parseCountries(searchParams);
    ...
    if (selectedCountries.length === 0) {
      return NextResponse.json({
        ...
        countries: [],
        points: [],
        emptyBecauseNoCountrySelected: true,
      });
    }

    const limit = getLimit(searchParams, 800);
    ...
    const points: MapPoint[] = (wines as WineRow[])
      ...
      .filter((w) => matchesSelectedCountries(w.country, selectedCountries))
      .map((w) => {
        const coords =
    ...
    return NextResponse.json({
      ...
      points: sorted,
      emptyBecauseNoCountrySelected: false,
    });
  } catch (error) {
